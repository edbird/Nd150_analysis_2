...
void thickness()
{
    ...
    TH2D *h_ratio_error_double_nominal = new TH2D("h_ratio_error_double_nominal", "h_ratio_error_double_nominal", 50, 0.0, 5.0, 50, 0.0, 5.0);
    TH2D *h_ratio_error_half_nominal = new TH2D("h_ratio_error_half_nominal", "h_ratio_error_half_nominal", 50, 0.0, 5.0, 50, 0.0, 5.0);
    TH2D *h_diff_double_nominal = new TH2D("h_diff_double_nominal", "h_diff_double_nominal", 50, 0.0, 5.0, 50, 0.0, 5.0);
    TH2D *h_diff_half_nominal = new TH2D("h_diff_half_nominal", "h_diff_half_nominal", 50, 0.0, 5.0, 50, 0.0, 5.0);
    for(Int_t j = 1; j <= h_ratio_double_nominal->GetNbinsY(); ++ j)
    {
        for(Int_t i = 1; i <= h_ratio_double_nominal->GetNbinsX(); ++ i)
        {
            ...
            if((content_nominal != 0.0) && (content_double != 0.0))
            {
                ...
                Double_t error = std::sqrt(std::pow(error_double / content_nominal, 2.0) + std::pow(error_nominal * content_double / (content_nominal * content_nominal), 2.0));
                h_ratio_double_nominal->SetBinError(i, j, error);
                h_ratio_error_double_nominal->SetBinContent(i, j, error);
            }
            if((content_nominal != 0.0) && (content_half != 0.0))
            {
                h_ratio_half_nominal->SetBinContent(i, j, content_half / content_nominal);
                Double_t error = std::sqrt(std::pow(error_half / content_nominal, 2.0) + std::pow(error_nominal * content_half / (content_nominal * content_nominal), 2.0));
                h_ratio_half_nominal->SetBinError(i, j, error);
                h_ratio_error_half_nominal->SetBinContent(i, j, error);
            }
            h_diff_double_nominal->SetBinContent(i, j, content_double - content_nominal);
            h_diff_half_nominal->SetBinContent(i, j, content_half - content_nominal);
        }
    }
    ...
    TFile *f = new TFile("systematic_foil_thickness.root", "RECREATE");
    h_sys->Write();
    f->Close();
    ...
    for(Int_t j = 1; j <= h_ratio_double_nominal->GetNbinsY(); ++ j)
    {
        ...
        {
            Double_t content_nominal = h_nominal->GetBinContent(i, j);
            if(content_nominal == 0.0) continue;
            Double_t error_double = h_ratio_error_double_nominal->GetBinContent(i, j);
            Double_t error_half = h_ratio_error_half_nominal->GetBinContent(i, j);
            Double_t error = std::sqrt(std::pow(error_double, 2.0) + std::pow(error_half, 2.0));
            Double_t h = 0.0 + error;
            Double_t l = 0.0 - error;
            h_sys_h->SetBinContent(i, j, h);
            h_sys_l->SetBinContent(i, j, l);
