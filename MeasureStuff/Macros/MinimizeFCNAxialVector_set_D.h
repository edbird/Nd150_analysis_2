...
MinimizeFCNAxialVector::set_D() const
{
    ...
    for(int channel = 0; channel < number2DHists; ++ channel)
    {
        ...
        if(g_mode_fake_data == false)
        {
            tmpDataHist2D_P1 = (TH2D*)allDataSamples2D->FindObject(search_object_P1.c_str());
            tmpDataHist2D_P2 = (TH2D*)allDataSamples2D->FindObject(search_object_P2.c_str());
        }
        else if(g_mode_fake_data == true)
        {
        ...
        //tmpData1D_P1 = new Double_t[tmpDataHist1D_P1->GetNbinsX()]; //tmpHistP1->Clone("tmpData1D_P1");
        //tmpData1D_P2 = new Double_t[tmpDataHist1D_P2->GetNbinsX()]; //tmpHistP2->Clone("tmpData1D_P2");
        for(Int_t bin_y{0}; bin_y < tmpDataHist2D_P1->GetNbinsY(); ++ bin_y)
        {
            for(Int_t bin_x{0}; bin_x < tmpDataHist2D_P1->GetNbinsX(); ++ bin_x)
            {
                Double_t content = tmpDataHist2D_P1->GetBinContent(bin_x + 1, bin_y + 1);
                #if VECTOR_RANGE_CHECK
                D_2D_P1_data[channel]->at(bin_x + 12 * bin_y) = content;
                #else
                D_2D_P1_data[channel]->operator[](bin_x + 12 * bin_y) =  content;
                #endif
            }
        }
        ...
        //std::cout << "LIST OF P2 DATA channel=" << channel << std::endl;
        //std::cin.get();
        for(Int_t bin_y{0}; bin_y < tmpDataHist2D_P2->GetNbinsY(); ++ bin_y)
        {
            for(Int_t bin_x{0}; bin_x < tmpDataHist2D_P2->GetNbinsX(); ++ bin_x)
            {
                Double_t content = tmpDataHist2D_P2->GetBinContent(bin_x + 1, bin_y + 1);
                #if VECTOR_RANGE_CHECK
                D_2D_P2_data[channel]->at(bin_x + 12 * bin_y) = content;
                #else
                D_2D_P2_data[channel]->operator[](bin_x + 12 * bin_y) = content;
                #endif
            }
        }
    }
