    ...
    for(int channel = 0; channel < number2DHists; ++ channel)
    {
        ...
        if(channel_enable_2D[channel] == 1)
        {
            ...
            if(recalculate_V_PHYS_MATHMORE == true)
            {
                ...
                int counter_P2 = 0;
                //for(Int_t binx{1}; binx <= M_1D_P1[channel]->GetNbinsX(); ++ binx)
                for(Int_t binx{0}; binx < M_2D_P1_data[channel]->size(); ++ binx)
                {
                    ...
                    // P1
                    {
                        ...
                        if(content_M <= 0.0)
                        {
                            V_ENABLE_BIN_2D_P1[channel]->push_back(false);
                        }
                        else
                        {
                            V_ENABLE_BIN_2D_P1[channel]->push_back(true);
                        ...
                        #if VECTOR_RANGE_CHECK
                        V_PHYS_STAT_2D_P1_data[channel]->at(binx * 12 * 12 + binx) = stat;
                        #else
                        V_PHYS_STAT_2D_P1_data[channel]->operator[](binx * 12 * 12 + binx) = stat;
                        #endif
                    ...
                    // P2
                    {
                        ...
                        if(content_M <= 0.0)
                        {
                            V_ENABLE_BIN_2D_P2[channel]->push_back(false);
                        }
                        else
                        {
                            V_ENABLE_BIN_2D_P2[channel]->push_back(true);
                        ...
                        #if VECTOR_RANGE_CHECK
                        V_PHYS_STAT_2D_P2_data[channel]->at(binx * 12 * 12 + binx) = stat;
                        #else
                        V_PHYS_STAT_2D_P2_data[channel]->operator[](binx * 12 * 12 + binx) = stat;
                        #endif
                    }
                }
                //std::cout << "P1: number of enabled bins = " << counter_P1 << std::endl;
                ...
                #endif
                {
                    if(V_PHYS_2D_P1_MATHMORE[channel] != nullptr)
                    {
                        if(counter_P1 != V_PHYS_2D_P1_MATHMORE[channel]->GetNrows())
                        {
                            // nrows always == ncols
                            delete V_PHYS_2D_P1_MATHMORE[channel];
                            V_PHYS_2D_P1_MATHMORE[channel] = new TMatrixD(counter_P1, counter_P1); // NUM_BINS_XY, NUM_BINS_XY);
                        }
                    }
                    else
                    {
                        std::cout << "MATHMORE: P1: " << V_PHYS_2D_P1_MATHMORE[0] << std::endl;
                        std::cout << "ALLOC " << __func__ << std::endl;
                        V_PHYS_2D_P1_MATHMORE[channel] = new TMatrixD(counter_P1, counter_P1); // NUM_BINS_XY, NUM_BINS_XY);
                    }

                    if(V_PHYS_2D_P2_MATHMORE[channel] != nullptr)
                    {
                        if(counter_P2 != V_PHYS_2D_P2_MATHMORE[channel]->GetNrows())
                        {
                            delete V_PHYS_2D_P2_MATHMORE[channel];
                            V_PHYS_2D_P2_MATHMORE[channel] = new TMatrixD(counter_P2, counter_P2); //NUM_BINS_XY, NUM_BINS_XY);
                        }
                    }
                    else
                    {
                        std::cout << "MATHMORE: P2: " << V_PHYS_2D_P2_MATHMORE[0] << std::endl;
                        std::cout << "ALLOC " << __func__ << std::endl;
                        V_PHYS_2D_P2_MATHMORE[channel] = new TMatrixD(counter_P2, counter_P2); //NUM_BINS_XY, NUM_BINS_XY);
                    }
                    // TODO: must be able to optimize this
                ...
                std::chrono::system_clock::time_point end_time = std::chrono::high_resolution_clock::now();
                std::chrono::duration<double> runtime_sec = end_time - start_time;
                std::cout << "V_PHYS_2D_Px_MATHMORE allocation time: " << 1.0e+06 * runtime_sec.count() << " microsecond" << std::endl;
                #endif
                ...
                int j_counter = 0;
                //for(Int_t j = 0; j < V_PHYS_STAT_1D_P1[channel]->GetNbinsY(); ++ j)
                for(Int_t j = 0; j < 12 * 12; ++ j)
                {
                    if(V_ENABLE_BIN_2D_P1[channel]->at(j) == true)
                    {
                        // do nothing
                    }
                    else
                    ...
                    int i_counter = 0;
                    for(Int_t i = 0; i < 12 * 12; ++ i)
                    {
                        if(V_ENABLE_BIN_2D_P1[channel]->at(i) == true)
                        {
                            // do nothing
                        }
                        else
                        ...
                        if(V_ENABLE_STAT == true)
                        {
                            ...
                            if(i == j)
                            {
                                cstat = V_PHYS_STAT_2D_P1_data[channel]->at(i + j * 12 * 12);
                            }
                        }
                        if(V_ENABLE_SYSALL == true)
                        {
                            csys = V_PHYS_SYSALL_2D_P1_data[channel]->at(i + j * 12 * 12);
                        }
                        #else
                        if(V_ENABLE_STAT == true)
                        {
                            ...
                            if(i == j)
                            {
                                cstat = V_PHYS_STAT_2D_P1_data[channel]->operator[](i + j * 12 * 12);
                            }
                        }
                        if(V_ENABLE_SYSALL == true)
                        {
                            csys = V_PHYS_SYSALL_2D_P1_data[channel]->operator[](i + j * 12 * 12);
                        }
                        #endif
                        content = cstat + csys;
                        //std::cout << "i=" << i << " j=" << j << " i_counter=" << i_counter << " j_counter=" << j_counter << " content=" << content << std::endl;
                        V_PHYS_2D_P1_MATHMORE[channel]->operator[](j_counter).operator[](i_counter) = content;
                        //std::cout << "j=" << j << " i=" << i << " " << content << std::endl;
                ...
                for(Int_t j = 0; j < 12 * 12; ++ j)
                {
                    if(V_ENABLE_BIN_2D_P2[channel]->at(j) == true)
                    {
                        // do nothing
                    }
                    else
                    ...
                    for(Int_t i = 0; i < 12 * 12; ++ i)
                    {
                        if(V_ENABLE_BIN_2D_P2[channel]->at(i) == true)
                        {
                            // do nothing
                        }
                        else
                        ...
                        if(V_ENABLE_STAT == true)
                        {
                            ...
                            if(i == j)
                            {
                                cstat = V_PHYS_STAT_2D_P2_data[channel]->at(i + j * 12 * 12);
                            }
                        }
                        if(V_ENABLE_SYSALL == true)
                        {
                            csys = V_PHYS_SYSALL_2D_P2_data[channel]->at(i + j * 12 * 12);
                        }
                        #else
                        if(V_ENABLE_STAT == true)
                        {
                            ...
                            if(i == j)
                            {
                                cstat = V_PHYS_STAT_2D_P2_data[channel]->operator[](i + j * 12 * 12);
                            }
                        }
                        if(V_ENABLE_SYSALL == true)
                        {
                            csys = V_PHYS_SYSALL_2D_P2_data[channel]->operator[](i + j * 12 * 12);
                        }
                        #endif
                        content = cstat + csys;
                        //std::cout << "i=" << i << " j=" << j << " i_counter=" << i_counter << " j_counter=" << j_counter << " content=" << content << std::endl;
                        V_PHYS_2D_P2_MATHMORE[channel]->operator[](j_counter).operator[](i_counter) = content;
                ...
                #endif
                //std::cout << "P1 V_MATRIX size: " << V_PHYS_1D_P1_MATHMORE[channel]->GetNrows() << " " << V_PHYS_1D_P1_MATHMORE[channel]->GetNcols() << std::endl;
                if(V_PHYS_2D_P1_MATHMORE[channel]->Determinant() == 0.0)
                {
                    std::cout << "P1 V_MATRIX size: " << V_PHYS_2D_P1_MATHMORE[channel]->GetNrows() << " " << V_PHYS_2D_P1_MATHMORE[channel]->GetNcols() << std::endl;
                    for(int j = 0; j < V_PHYS_2D_P1_MATHMORE[channel]->GetNcols(); ++ j)
                    {
                        for(int i = 0; i < V_PHYS_2D_P1_MATHMORE[channel]->GetNrows(); ++ i)
                        {
                            std::cout << "i=" << i << " j=" << j << " " << V_PHYS_2D_P1_MATHMORE[channel]->operator[](j).operator[](i) << std::endl;
                        }
                    }
                }

                // invert
                V_PHYS_2D_P1_MATHMORE[channel]->Invert();
                V_PHYS_2D_P2_MATHMORE[channel]->Invert();
